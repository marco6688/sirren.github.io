...
大多数情况都可以确定某次调用应该选择那个函数，然而当几个重载函数的形参数量相等以及某些形参的类型可以由其他类型转换而来时，这时函数的选择也会有所不同

1. 确认候选函数和可行函数

	1. 函数匹配的第一步是选定不辞调用对应的重载函数集，集合中的函数称为**候选函数**，该函数具有两个特征：一与被调用函数同名，二其声明在调用点可见
	2. 第二步考察本次调用实参，然后从候选函数中选出能被这组实参调用的函数，这些新选出来的函数称为**可行函数**，这类函数也有两个特征：一其形参数量与本次调用提供的实参数量相等，二是每个实参的类型与对应形参的类型相同，或者能转换成形参的类型。

2. 寻找最佳匹配
	
	这一过程逐一检查调用提供的形参，寻找形参类型与灿灿类型最匹配的函数，他们类型越接近匹配的越好
	
3. 含有多个形参的函数匹配
	
	1. 该函数的每个形参都不劣与其他可行函数的匹配
	2. 至少有一个实参的匹配优于其他匹配

## 实参类型的转换

为了确定最佳匹配，编译器将实参类型到形参类型的转换分为了几个等级：

1. 精确匹配，以下几种情况：

	1. 实参类型与形参类型完全相同
	2. 实参从数组类型或函数类型转换成对应的指针
	3. 向实参添加顶层const或从实参删除顶层const
...
函数指针指向的是函数而非对象。函数的类型是由返回类型和形参共同决定的，与函数名无关
...
//声明一个函数指针，func指向一个函数
bool （*func2）(const string &);//未初始化

//当我们把函数名当做值来使用时，该函数自动转换为指针。
func2 = func;
func2 = &func;//等价赋值语句，取址符是可选的

//调用过程，我们直接调用指针，无须解引用
bool s1 = func2("hello");
bool s2 = (*func2)("hello");//等价直接调用
bool s3 = func1("hello");

//重载函数指针，必须清晰的界定到底应该选择用那个函数

void ff(int *);
void ff(unsigned int);

void (*f1)(unsigned int) = ff;//指向ff(unsigned)

void (*f1)(int) = ff;			//错误，形参不匹配
double (*f1)(unsigned int)=ff;//错误，返回恶习不匹配

```

* 函数和数组一样，不能定义实参，但是可以把指针传递过去
* 同时也可以返回函数指针
