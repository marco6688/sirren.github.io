...
**函数匹配**是指我们把函数调用与一组函数的某一个关联起来，函数匹配也叫做**重载确定**
匹配会得到三种结果：

1. 编译器找到一个**最佳匹配**结果，并生成该调用函数
2. 找不到任何一个函数与调用的实参匹配，编译器发出**无匹配**的错误
3. 存在多个函数可以匹配，但是每一个都不是最佳选择，此时也会发生错误，称为**二义性调用** 
...
如果我们在内层作用域声明名字，他讲隐藏外层作用域中声明的同名实体，在不同作用域中无法重载函数名

# 特殊用途语言特性

## 默认实参

某些函数在很多次调用的是=时候赋予了一个相同的值，此时我们把这个反复出现的值称为**默认实参**，在调用含有默认实参的函数时，可以省略这个形参

在给定作用域中一个形参只能被赋予一次默认实参。函数的后续声明只能为那些没有默认值的形参添加默认形参，而且形参右侧的所有形参都必须有默认值。

## 内联函数

内联函数可避免韩式调用的开销
将函数指定为内联函数，通常是将他的每个调用点上展开，将函数返回类型前添加inline就可以将其声明为内联函数

## constexpr函数


constexpr函数是指能用于常量表达式的函数，定义函数的方法与其他函数类似，不过要遵循几项约定：函数的返回类型及所有形参的类型必须是字面值类型，而且函数体必须有且只有一条return语句

## 调试帮助

C++程序员有时候会遇到一种类似头文件保护的技术，以便有选择的执行调试代码。基本思想是程序开发时包含一些调试代码。等到程序发布时要先屏蔽掉调试代码。

### assert 预处理宏

assert宏位于cassert头文件中，在包含了这个头文件或隐性的包含了这个头文件都不能使用assert的变量、函数或其他实体。
assert 宏用于检查“不能发生”的条件，条件返回假assert输出信息终止程序，返回真assert什么也不做

### NDEBUG 预处理变量

assert依赖于NDEBUG的预处理变量，如果定义了这个变量，assert什么也不做。默认状态下没有定义这个变量，assert执行检查

可以说使用#define 语句定义NDEBUG，从而关闭调试状态，同时编译器提供了一个选项也可以定义这个变量
> CC -D NDEBUG main.c # 使用 -D 来定义NDEBUG变量

我们还可以使用NDEBUG来写自己的调试代码，以下示例

```

void print(){

//如果未定义NDEBUG将执行#ifdef 和 #endif之间的代码
#ifdef NDEBUG
	//__func__是静态局部变量，用于存放函数名称
	//__FILE__存放文件名的字面值
	//__LINE__当前行号的字面值
	//__TIME__编译时间的字面值
	//__TATE__编译日期的字面值
	cerr<< __func__<<": is function name"<<endl;
